...
--4.Show the employee_id,order_id,customer_id,required_date,
---shipped_date from all orders shipped later than the required
--date.
...
--6. Show the city,company_name,contact_name of all customers from
---cities which contains the letter 'L' in the city name,sorted by
---contact_name.
...
--7.Show the company_name,contact_name,fax_number of all customers
--that has a fax number. (not null)
SELECT company_name,contact_name,fax
FROM customers
WHERE fax IS NOT NULL;

--8. Show the first_name,last_name,hire_date of the most recently
---hired employee.
SELECT first_name,last_name,MAX(hire_date)
FROM  employees;

--9. Show the average unit price rounded to 2 decimal places, the total
--units in stock, total discontinued products from the products table.
SELECT ROUND(AVG(unit_price),2) AS average_price, SUM(units_in_stock)
